...
class DbOperations:
    ...
    def create_question(self):
        self.conn.execute("CREATE TABLE question_data (id INTEGER PRIMARY KEY AUTOINCREMENT, question LONGTEXT NOT NULL, \
        ...
        for question in questions:
            ...
            self.conn.execute(
                f"""insert into question_data(`question`,`options`,`correct_ans`) values("{ques}","{options}","{correct_ans}")""")
            self.conn.commit()
    ...
    def get_questions(self):
        cursor = self.conn.execute("select * from question_data")
