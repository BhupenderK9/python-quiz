...
def register(request):
    ...
    user_list = DbOperations().user_exist_check(username=username)
    if user_list:
        context = {'authorized': False, 'message': 'User Already Exist, please login'}
        return render(request, "polls/index.html", context)
...
def take_test(request):
    ...
    answer = form.get('answer') # A hidden element which has the answer value, question.answer
    username = form.get('username')
    
    #if count == 0 then pull the questions 
    if int(count) == 0:
    ...
    message = ''
    if int(count) < 5:
        current_question = array[int(count)] # Add the question from array to here
        # check answer
        if int(count) != 0:
        ...
        display_quiz = False # make this false to stop viewing the quiz
    ...
    context = {
        ...
        'question': current_question, # pass the whole list which has answer and everything in place
        'username': username,
        'count': count, # Updating the count
...
def login_redirect(request):
    context = {'authorized': False, 'message': ''}
    return render(request, "polls/index.html", context)
